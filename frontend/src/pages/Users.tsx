    ...
    <div className="font-body-md text-on-surface dark:text-white antialiased space-y-6 bg-[#fff0f3] dark:bg-transparent min-h-screen p-4 sm:p-6 -m-4 sm:-m-6 transition-colors duration-300">
      <div className="max-w-7xl mx-auto space-y-6 pb-12">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <h1 className="font-h2 text-3xl font-bold text-on-background dark:text-white tracking-tight">
            User Management
          </h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-white dark:bg-[#1a1a1c] rounded-2xl p-6 shadow-sm border border-gray-100 dark:border-gray-800 relative overflow-hidden group hover:shadow-md transition-all duration-300">
            ...
            <h3 className="relative z-10 text-4xl font-bold text-[#1e293b] dark:text-white">
        ...
        <section className="bg-surface-container-lowest dark:bg-[#1a1a1c] rounded-2xl shadow-sm border border-outline-variant/20 dark:border-gray-800 overflow-hidden transition-colors duration-300">
              ...
              {showFilterDropdown && (
                <div className="absolute top-full right-0 mt-2 w-[240px] bg-surface-container-lowest dark:bg-[#1a1a1c] border border-outline-variant/20 dark:border-gray-800 rounded-xl shadow-xl z-20 p-4">
                  <div className="space-y-4">
                    <div>
                      <label className="block text-[10px] font-bold text-secondary dark:text-gray-400 mb-1.5 uppercase tracking-wider">
                        By Role
                      </label>
                      <select
                        value={roleFilter}
              ...
              <tbody className="text-on-surface dark:text-gray-300 text-sm divide-y divide-surface-container dark:divide-gray-800">
                {isLoading ? (
                  <tr>
                    <td
                      colSpan={4}
                      className="px-6 py-12 text-center text-secondary dark:text-gray-500"
                    >
                  ...
                  currentUsers.map((u) => (
                    <tr
                      key={u.id}
                      className="hover:bg-red-50/50 dark:hover:bg-gray-800/50 transition-colors group"
                    >
                      ...
                      <td className="px-6 py-4">
                        <span
                          className={`inline-flex items-center px-3 py-1 rounded-full text-[11px] font-bold tracking-wide uppercase shadow-sm border ${
                            u.role === "admin"
                        ...
                        >
                          {u.role === "admin" ? "Super Admin" : "Admin"}
                        </span>
                      </td>
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-2">
                          <span
                            className={`w-2 h-2 rounded-full ${u.is_active ? "bg-emerald-500" : "bg-gray-400 dark:bg-gray-600"}`}
                          ></span>
                          <span
                            className={`text-sm font-bold ${u.is_active ? "text-emerald-600 dark:text-emerald-400" : "text-secondary dark:text-gray-500"}`}
                          >
          ...
          <div className="px-6 py-4 border-t border-surface-container dark:border-gray-800 bg-surface-container-low/30 dark:bg-[#202022] flex flex-col sm:flex-row items-center justify-between gap-4">
            <p className="text-sm text-secondary dark:text-gray-400">
              Showing{" "}
              <span className="font-bold text-on-background dark:text-white">
              ...
              <div className="flex justify-between items-center px-6 py-4 border-b border-outline-variant/20 dark:border-gray-800 bg-surface-container-low/50 dark:bg-[#202022]">
                <div>
                  ...
                  <p className="text-xs text-secondary dark:text-gray-400 mt-0.5">
                    {editingUser
                      ? "Update user credentials and access level."
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="p-6">
                <div className="space-y-5">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                      <label className="block text-sm font-semibold text-on-surface dark:text-gray-300 mb-1.5">
                      ...
                      />
                      {errors.name && (
                        <p className="text-primary text-xs mt-1.5 font-medium">
                          {errors.name.message}
                        </p>
                      )}
                    </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                      <label className="block text-sm font-semibold text-on-surface dark:text-gray-300 mb-1.5">
                        System Role <span className="text-primary">*</span>
                      </label>
                      <select
                        {...register("role")}
                    ...
                    <div>
                      <label className="flex text-sm font-semibold text-on-surface dark:text-gray-300 mb-1.5 items-center justify-between">
                        <span>
                          Password{" "}
                          {!editingUser && (
                        ...
                        {editingUser && (
                          <span className="text-[10px] uppercase tracking-wider text-secondary dark:text-gray-500 font-normal">
                            Optional
                          </span>
                        )}
                      </label>
                  ...
                  <div className="pt-2">
                    <label className="flex items-center p-3 border border-outline-variant/50 dark:border-gray-700 rounded-lg cursor-pointer hover:bg-surface-container-low dark:hover:bg-[#252525] transition-colors">
                      ...
                      <div className="ml-3">
                        <span className="block text-sm font-semibold text-on-background dark:text-white">
                          Active Account
                        </span>
                        <span className="block text-xs text-secondary dark:text-gray-400 mt-0.5">
