...
function getPeriodOptions(): PeriodOption[] {
  ...
  return [
    // Tambahan Opsi "Hari ini / Now"
    {
      label: "Today",
      startDate: fmt(now),
      endDate: fmt(now),
      chartTitle: "Daily Trends (Today)",
    },
    {
      label: "Last 7 Days",
      startDate: fmt(last7),
      endDate: fmt(now),
      chartTitle: "Daily Trends (Last 7 Days)",
    },
    {
      label: "This Month",
      startDate: fmt(monthStart),
      endDate: fmt(now),
      chartTitle: "Daily Trends (This Month)",
    },
    {
      label: "This Year",
      startDate: fmt(yearStart),
      endDate: fmt(now),
      chartTitle: "Monthly Trends (This Year)",
    },
    {
      label: "All Time",
      startDate: "",
      endDate: "",
      chartTitle: "All Time Trends",
    },
  ];
}
...
export default function Reports() {
  ...
  // Update state initial value: 2 (merujuk pada "This Month") atau sesuaikan dengan keinginan (misal 0 untuk "Today")
  ...
  const queryParams = period.startDate
  ...
  const { data: summary, isLoading: summaryLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await api.get(
        `/reports/summary${queryParams ? "?" + queryParams : ""}`,
      );
  ...
  const { data: txData, isLoading: txLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await api.get(
        `/reports/transactions?${queryParams ? queryParams + "&" : ""}page=${page}&limit=5`,
      );
      return res.data.data;
    },
  });
  ...
  const handleExportExcel = async () => {
    ...
    try {
      // Menambahkan periodLabel ke URL agar backend juga tahu
      const res = await api.get(
        `/reports/export/excel${queryParams ? "?" + queryParams : ""}${queryParams ? "&" : "?"}periodLabel=${encodeURIComponent(period.label)}`,
        { responseType: "blob" },
      );
      ...
      // 👇 INI KUNCI UTAMANYA: Ubah nama file di sini agar dinamis
      link.setAttribute("download", `Report ${period.label}.xlsx`);
  ...
  const handleExportPdf = async () => {
    ...
    try {
      // Menambahkan periodLabel ke URL agar backend juga tahu
      const res = await api.get(
        `/reports/export/pdf${queryParams ? "?" + queryParams : ""}${queryParams ? "&" : "?"}periodLabel=${encodeURIComponent(period.label)}`,
        { responseType: "blob" },
      );
      const url = window.URL.createObjectURL(new Blob([res.data]));
      const link = document.createElement("a");
      ...
      // 👇 INI KUNCI UTAMANYA: Ubah nama file di sini agar dinamis
      link.setAttribute("download", `Report ${period.label}.pdf`);
  ...
  const pieData =
    summary?.spendingByCategory?.map(
      (c: { category_name: string; total: number }) => ({
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
                Analyze spending trends and export data
              </p>
            </div>
          </div>
          <div className="flex items-center gap-3">
          ...
          <div className="bg-white dark:bg-[#1a1a1c] rounded-2xl p-6 border border-gray-100 dark:border-gray-800 shadow-sm relative overflow-hidden group hover:shadow-md transition-all">
            ...
            <div className="relative z-10">
              ...
              <p className="text-4xl font-bold text-[#1e293b] dark:text-white">
                Rp {(summary?.totalSpend ?? 0).toLocaleString("id-ID")}
              </p>
            </div>
          </div>
          <div className="bg-white dark:bg-[#1a1a1c] rounded-2xl p-6 border border-gray-100 dark:border-gray-800 shadow-sm relative overflow-hidden group hover:shadow-md transition-all">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-4 gap-4">
              ...
              <div className="flex gap-2">
                ...
                >
                  ...
                  <span>PDF</span>
                  <div className="w-2 h-2 bg-[#a60016] rounded-full ml-1" />
                </button>
              </div>
            </div>
            {summaryLoading ? (
              <div className="h-52 flex items-center justify-center">
                <Loader2 className="w-8 h-8 animate-spin text-[#a60016]" />
              </div>
            ) : pieData.length === 0 ? (
              <div className="h-52 flex items-center justify-center text-gray-400 font-medium text-sm bg-gray-50 dark:bg-[#252525] rounded-xl border border-dashed border-gray-200 dark:border-gray-700 m-2">
                No data available
              </div>
            ) : (
              <>
                ...
                <div className="flex flex-wrap justify-center gap-x-5 gap-y-3 mt-4 px-2">
                  {pieData.map(
                    (entry: { name: string; value: number }, idx: number) => {
                      ...
                      return (
                        <div key={idx} className="flex items-center gap-2">
                          ...
                          <span className="text-[13px] font-medium text-gray-600 dark:text-gray-400 whitespace-nowrap">
                            {entry.name}{" "}
                            <span className="font-bold text-[#1e293b] dark:text-white ml-1">
                              {pct}%
                            </span>
                          </span>
                        </div>
          ...
          <div className="bg-white dark:bg-[#1a1a1c] rounded-[1.5rem] p-6 border border-gray-100 dark:border-gray-800 shadow-sm">
            <div className="flex items-start justify-between mb-6">
              ...
              <div className="text-right">
                <p className="text-xl font-bold text-[#a60016]">
                  Rp {(summary?.totalSpend ?? 0).toLocaleString("id-ID")}
                </p>
                <p className="text-[11px] font-bold uppercase tracking-widest text-gray-400 dark:text-gray-500 mt-1">
                  Total Spend
                </p>
              </div>
            </div>
            {summaryLoading ? (
              ...
              <div className="h-[240px] flex items-center justify-center text-gray-400 font-medium text-sm bg-gray-50 dark:bg-[#252525] rounded-xl border border-dashed border-gray-200 dark:border-gray-700 m-2">
                No data available
              </div>
            ) : (
              <ResponsiveContainer
                ...
                {txLoading ? (
                  <tr>
                    <td colSpan={6} className="px-6 py-12 text-center">
                      <Loader2 className="w-8 h-8 animate-spin mx-auto text-[#a60016]" />
                    </td>
                  </tr>
                ) : !txData?.notes?.length ? (
                  <tr>
                    <td
                      colSpan={6}
                      className="px-6 py-12 text-center font-medium text-gray-500 dark:text-gray-400"
                    >
                  ...
                  txData.notes.map(
                    (n: {
                      ...
                      <tr
                        key={n.id}
                        className="hover:bg-gray-50/80 dark:hover:bg-[#202022] transition-colors bg-white dark:bg-[#1a1a1c] border-b border-gray-100 dark:border-gray-800 last:border-0"
                      >
                        ...
                        <td className="px-6 py-4 text-right font-bold text-[#1e293b] dark:text-white text-[15px]">
            ...
            <div className="px-6 py-5 border-t border-gray-100 dark:border-gray-800 flex items-center justify-between text-sm bg-white dark:bg-[#1a1a1c]">
              <div className="text-sm text-gray-500 dark:text-gray-400 font-medium">
                ...
                <span className="hidden sm:inline">
                  ({txData.pagination.total} receipts)
                </span>
              </div>
